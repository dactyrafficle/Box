/* LAST UPDATED : 2024-06-26-0010 EDT */
...
function Box() {
  ...
  this.data.dimension = {};
  this.data.dimension.w = pixel_width;
  this.data.dimension.h = pixel_height;
...
// ALL THIS DOES IS CHANGE THE PIXEL WIDTH AND HEIGHT OF THE BOX
Box.prototype.RESIZE = function(w_, h_) {
...
Box.prototype.RESCALE_BASED_ON_CM = function(obj) {
...
Box.prototype.SHOW_GRID_X = function(dx_=1, line_width_=1) {

  let x_start = Math.floor(this.data.range.x.min/dx_)*dx_;
  
  let y0 = this.data.range.y.min;
  let y1 = this.data.range.y.max;

  this.ctx.lineWidth = line_width_;

  for (let x = x_start; x <= this.data.range.x.max; x += dx_) {
  ...
  /*
  ctx.lineWidth = 1;
  ctx.beginPath();      // DECLARES A LINE WILL BE DEFINED
  ctx.moveTo(50, 75);   // DEFINES THE FIRST POINT
  ctx.lineTo(200, 75);  // DEFINES A SUBSEQUENT POINT
  ctx.stroke();         // DRAWS THE LINE
  */
};


Box.prototype.SHOW_GRID_Y = function(dy_=1, line_width_=1) {

  let y_start = Math.floor(this.data.range.y.min/dy_)*dy_;
  ...
  for (let y = y_start; y <= this.data.range.y.max; y += dy_) {
...
Box.prototype.ARROW = function(obj) {
  ...
  */
  ...
  // p1 is surely one of the points of the triangle
  // what are the other 2 points ? p2 and p3
  // if rho_outer is the distance between p0 and p1
  // if i make a circle from p0 with radius rho_outer
  // then p2 and p3 will always be within that circle
  
  // let us then make a smaller circle, with rho_inner = 0.9*rho_outer
  
  let v5 = {};
  v5.x = v0.x + nx*mag*0.9;
  v5.y = v0.y + ny*mag*0.9;
  
  let v2 = {};
  v2.x = v5.x + nx_perp*mag*0.04;
  v2.y = v5.y + ny_perp*mag*0.04;
  
  let v3 = {};
  v3.x = v5.x - nx_perp*mag*0.04;
  v3.y = v5.y - ny_perp*mag*0.04;

  let p0 = this.VALUE2PIXEL(v0);  // the origin
  let p1 = this.VALUE2PIXEL(v1);  // the target
  let p2 = this.VALUE2PIXEL(v2);
  let p3 = this.VALUE2PIXEL(v3); 
 
  this.ctx.lineWidth = obj.line_width;
  this.ctx.strokeStyle = obj.stroke_style;
  //this.ctx.fillStyle = obj.fill_style;
...
//
Box.prototype.POINT2 = function(x, y, rho, color) {

 /* val = {'x':x,'y':y} */
 /* this.RADIUS(rx) */

 let p = this.VALUE2PIXEL({'x':x, 'y':y});
 let r = rho;
...
// CES UTILITY
// u=(a1*(b1*x1)**delta+a2*(b2*x2)**delta)**(1/delta)
...
Box.prototype.SHOW_CES_INDIFFERENCE_CURVE = function(obj) {
  ...
  let x1_min = 0;
  let x2_min = 0;
  let x1_max = this.data.range.x.max;
  let x2_max = this.data.range.y.max;
  ...
  if (!obj.u && (obj.x && obj.y)) {
    ...
    if (output.type === "ELSE") {u = (alpha*(b1*x)**delta + beta*(b2*y)**delta)**delta_inv};
  ...
  for (let i = 0; i <= 50; i++) {
    ...
    // THE ELSE CASES **
    if (u**delta >= alpha*(b1*x_index)**delta) {
      arr.push({
        'x':x_index,
        'y':((u**delta - alpha*(b1*x_index)**delta)/(beta*(b2**delta)))**delta_inv
      });
    }
    
    if (u**delta >= beta*(b2*y_index)**delta) {
      arr.push({
        'x':((u**delta - beta*(b2*y_index)**delta)/(alpha*(b1**delta)))**delta_inv,
        'y':y_index
      });
    }
...
/*
  let obj = {
    'arr':arr,  required. Array.isArray(arr)
    'number_of_bins':8,
    'lower_bound':-3,
    'upper_bound':3
...
// 2022-08-12 : histogram is a child class of box class. it has only one unique method : DRAW()
// 2022-04-07 : right now, only the minimal version works. the minimal version

class Histogram extends Box {

  constructor(obj) {
    super(obj);

    this.arr = obj.arr; // required
    this.bin_width;
    this.number_of_bins;
    this.global_range;
    this.global_lower_bound;
    this.global_upper_bound;
    this.bins = [];

    this.SET_RANGE(obj);
    this.SET_NUMBER_OF_BINS(obj);
    this.CREATE_BINS();
    this.SORT_DATA_INTO_BINS();
...
}; // closing histogram child class definition

Histogram.prototype.UPDATE = function(obj) {
...
// THE DEFAULT IS THAT THE LOWEST NUMBER IN THE ARRAY DEFINES THE HARD LOWER_BOUND
// 

Histogram.prototype.SET_RANGE = function(obj) {
  ...
  this.RANGE_X(-this.bin_width + this.global_lower_bound, this.global_upper_bound + this.bin_width);          
  this.RANGE_Y(-5, 100 + 5);
  
};

Histogram.prototype.SET_NUMBER_OF_BINS = function(obj) {
...
Histogram.prototype.CREATE_BINS = function() {
  for (let i = 0; i < this.number_of_bins; i++) {
    this.bins.push({
     'bin_index':i,
     'bin_lower_bound':this.global_lower_bound + i*this.bin_width,
...
Histogram.prototype.SORT_DATA_INTO_BINS = function() {
...
Histogram.prototype.DRAW = function() {
  ...
  // DRAW A RECTANGLE
  this.LINE_WIDTH(2);
  this.STROKE_STYLE('#99B3E6');
  this.FILL_STYLE('#D6E0F5aa');
...
Box.prototype.rbinom = function(k_, p_) {
  let sum = 0;
  for (let i = 0; i < k_; i++) {
    if (Math.random() < p_) {
      sum += 1;
...
let raf = (function() {
  ...
  function TextBox(obj) {
    ...
    */
    ...
    this.el.style.border = (obj.border || '1px solid black');
  ...
  Box.prototype.ADD_TEXT = function(TextBox) {
    ...
    if (TextBox.alignment === 'center-left') {
      let h = TextBox.el.getBoundingClientRect().height;
      let w = TextBox.el.getBoundingClientRect().width;
      TextBox.el.style.top = (pixel.y - 0.5*h) + 'px';
      TextBox.el.style.left = (pixel.x) + 'px';
